...
import { ShoppingBag, ChevronRight, Loader2, AlertCircle, CheckCircle2, Clock, XCircle } from 'lucide-react';
...
const Orders: React.FC = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(`Error ${res.status}: ${errorText || 'Failed to fetch orders'}`);
            }
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            case 'SHIPPED': return <CheckCircle2 size={14} className="text-green-500" />;
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="font-bold text-gray-500 uppercase tracking-widest text-xs">Retrieving your orders...</p>
            </div>
        );
    }
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
                ...
                <div className="px-4 py-2 bg-white rounded-xl border border-gray-100 shadow-sm">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Total History</p>
                    <p className="text-lg font-black text-gray-900 leading-none mt-1">{orders.length}</p>
                </div>
            </div>
            ...
            {orders.length === 0 ? (
                <div className="bg-white rounded-[2rem] p-16 flex flex-col items-center text-center border border-gray-100">
                    ...
                    <p className="text-gray-500 font-medium max-w-xs mb-8">Ready to start printing? Check out our latest products and place your first order.</p>
                    <button
                        onClick={() => navigate('/')}
                ...
                <div className="grid gap-4">
                    {orders.map((order) => (
                        <div key={order.id} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 font-bold mt-0.5">{new Date(order.createdAt).toLocaleDateString()}</p>
                                    </div>
                                </div>

                                <div className="grid grid-cols-2 md:grid-cols-1 gap-4 md:gap-1 text-right">
                                    <p className="text-xs text-gray-400 font-black uppercase tracking-widest md:hidden text-left">Total Amount</p>
                                    <p className="text-lg font-black text-gray-900">₹{order.totalAmount.toLocaleString()}</p>

                                    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase md:justify-self-end w-fit ${order.status === 'SHIPPED' ? 'bg-green-50 text-green-700' :
                                ...
                                <div className="flex items-center gap-2 pt-4 md:pt-0 border-t md:border-t-0 border-gray-50">
                                    ...
                                    {(order.status === 'CREATED' || order.status === 'PENDING') && (
                                        <button
                                            ...
                                            title="Cancel Order"
                                        >
                                            {cancellingId === order.id ? <Loader2 className="animate-spin" size={18} /> : <XCircle size={18} />}
                                        </button>
                                    )}
                                </div>
                            </div>
